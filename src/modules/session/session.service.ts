...
interface ExpiringDocumentCheck {
  documentType: string;
  expiryDate: string;
...
export interface ValidationResult {
  ...
  expiringDocuments: ExpiringDocumentCheck[];
...
export class SessionService {
  ...
  async getSessionSummary(sessionId: string): Promise<SessionSummary> {
    const session = await this.findSessionByIdWithExtractions(sessionId);
  ...
  async getExpiringDocuments(sessionId: string, withinDays: number): Promise<ExpiringAlertResult[]> {
    const sessionExists = await this.findSessionByIdWithExtractions(sessionId);
  ...
  async validateSessionData(sessionId: string): Promise<ValidationResult> {
    ...
    try {
      ...
      const result: ValidationResult = {
        ...
        expiringDocuments: (Array.isArray(parsedData.expiringDocuments) ? parsedData.expiringDocuments : []) as ExpiringDocumentCheck[],
